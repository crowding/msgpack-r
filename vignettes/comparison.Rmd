...
json <- timeCurve(timeout=10, dataset, timeConvert, fromJSON, toJSON)
...
timeConvert(dataset, reader=jsonlite_reader, writer=jsonlite_writer)
...
timeFileIO(subsample(dataset, 0.1), writer = writeMsg, reader = readMsg, wrap = msgConnection)
timeFifoTransfer(subsample(dataset, 0.1), writer = writeMsg, reader = readMsg, wrap = msgConnection)

## There appears to be some O(N^2) behavior with msgpack with
## connections, but not in memory.
...
## callback to reallocate the buffer and keep reading, making it an actually
## reentrant API.
```
