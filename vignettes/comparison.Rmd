...
* *JSON* is widely used on the web, but because it is based on
  ...
  representing JSON objects in R, and vice verse.
* *CSV* is ubiquitous and can be read by most anything but only represents
  tabular data. Its data types are ambiguous.
* *Msgpack* is similar in capabilities to JSON but is a binary
  format. On web applications, replacing JSON with msgpack can easily
  yield. I'm considering msgpack as a convenient wire format for
  embedded devices (IoT and such).
...
All that said.... Which is fastest? Which is most efficient? I'll give
more details below, but first I'll show the results. In both graphs,
lower is better.
...
details). In these cases I need to make sure all data is read before
starting to decode.
...
calls if the object is serialized into memory before sending.

Now I'll start collecting benchmarks systematically. Since the methods
we will explore have such a wide variance or performance, we will test
them with successively larger datasets, until they exceed a
timeout. The `serialize.spec` data structure annotated below
specifies how to test R serialization and how to label the
results. See the code file [benchmarking.R](benchmarking.R) for
more details.

```{R serialization}
# A spec is a named list of test factors.
# A test factor is a named list of arguments that will be passed to the
# test function. Annotated:
...
`rjson` is rather zippy -- only `r inmemory.factor %digits% 2` times
slower than `msgpack`, in memory, and `r remote.factor %digits% 2`
times slower across the wire. It does not support streaming reads, so
we must byte-buffer to read from connections. But that turns out to be
quite fast as well.
