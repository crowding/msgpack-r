...
```{R definitions, cache = FALSE, echo=FALSE}
read_chunk('benchmarking.R')
``` 
...
```{R}
(test.plot + labs("dput performance")) %+% filter(benchmarks, encoder == "dput") 
```
...
```{R}
(test.plot + labs("jsonlite performance")) %+% filter(benchmarks, encoder == "jsonlite")
```
...
```{R}
(test.plot + labs("msgpack performance")) %+% filter(benchmarks, encoder == "msgpack")
```
...
```{R}
(test.plot + labs("jsonlite performance")) %+% filter(benchmarks, encoder == "msgpackR")
```
...
```{R}
(test.plot + labs("rjson performance")) %+% filter(benchmarks, encoder == "rjson")
```
...
Oddly, RJSONIO's streaming function is quote slow.

```{R}
store(RJSONIO.timings)
```
```{R}
(test.plot + labs("RJSONIO performance")) %+% filter(benchmarks, encoder == "RJSONIO")
```
## ndjson
